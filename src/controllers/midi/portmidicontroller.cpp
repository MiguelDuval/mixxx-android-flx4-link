...
constexpr int kMidiStreamingSubclass = 0x03;
...
uint8_t midiCinForStatus(uint8_t status, int length) {
    ...
    if (status >= 0xF0) {
        if (status == 0xF0) {
            return length == 1 ? 0x5 : (length == 2 ? 0x6 : 0x4);
        }
...
bool PortMidiController::matchMapping(const MappingInfo& mapping) {
    ...
    for (const auto& product : products) {
        if (m_vendorId && m_productId &&
            ...
            if (product.interface_number.isEmpty() ||
...
int PortMidiController::open(const QString& resourcePath) {
    ...
    setOpen(true);
    qCInfo(m_logBase) << "Opened Android USB MIDI device" << getName();
    return 0;
}
...
bool PortMidiController::parseUsbMidiPacket(const uint8_t* packet, int packetSize) {
    ...
    if (length <= 0 || packetSize < 1 + length) {
        return false;
    }
    const uint8_t* data = packet + 1;
    const auto timestamp = mixxx::Duration::fromMillis(m_timestamp.elapsed());
    if (length >= 3) {
        receivedShortMessage(data[0], data[1], data[2], timestamp);
    } else if (length == 2) {
        receivedShortMessage(data[0], data[1], 0, timestamp);
    } else {
        receivedShortMessage(data[0], 0, 0, timestamp);
    }
    return true;
}
...
bool PortMidiController::sendBytes(const QByteArray& data) {
    ...
    while (offset < data.size()) {
        ...
        uint8_t cin = 0x4;
        if (isLast) {
            cin = length == 1 ? 0x5 : (length == 2 ? 0x6 : 0x7);
        }
        const auto* bytes = reinterpret_cast<const uint8_t*>(data.constData() + offset);
        if (!sendUsbMidiPacket(cin, bytes, length)) {
            return false;
        }
        offset += length;
    }
...
void PortMidiController::sendShortMsg(unsigned char status, unsigned char byte1,
    ...
    if (err != pmNoError) {
        qCWarning(m_logOutput) << "Error sending short message:" << Pm_GetErrorText(err);
    }
}
