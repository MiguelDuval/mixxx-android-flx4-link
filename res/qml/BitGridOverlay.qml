...
Item {
    ...
    // Native Mixxx BeatGrid actions. These are deliberately direct controls;
    // QML is only the UI layer and never implements its own grid math.
    ...
    property var state: activeDeck === 1 ? {
        loaded: trackLoaded1.value > 0,
        bpm: bpm1,
        phase: phase1,
        lock: lock1,
        undoPossible: undoPossible1,
        faster: faster1,
        slower: slower1,
        earlier: earlier1,
        later: later1,
        ...
        tap: tap1,
        halve: halve1,
        twoThirds: twoThirds1,
        threeFourths: threeFourths1,
        fourFifths: fourFifths1,
        fiveFourths: fiveFourths1,
        fourThirds: fourThirds1,
        threeHalves: threeHalves1,
        doubleBpm: double1
    } : {
        ...
        threeFourths: threeFourths2,
        fourFifths: fourFifths2,
        fiveFourths: fiveFourths2,
        fourThirds: fourThirds2,
        threeHalves: threeHalves2,
        doubleBpm: double2
    }

    property bool controlsInitialized: state.bpm.initialized && state.phase.initialized && state.lock.initialized
    property bool canEdit: controlsInitialized && state.loaded && state.lock.value < 0.5
    ...
    function bpmText(proxy) {
        if (!proxy.initialized || !state.loaded) {
            return "—"
        }
    ...
    // Compact entry buttons. They no longer mutate the grid themselves:
    // they open the full native BeatGrid editor for the selected deck.
    Row {
        id: entryBar
        ...
        Column {
            ...
            Rectangle {
                ...
                Row {
                    ...
                    Column {
                        ...
                        Text { color: Theme.lightGray1; font.bold: true; font.family: Theme.fontFamily; font.pixelSize: 20; text: root.bpmText(root.state.bpm) }
                    }
                    ...
                    Column {
                        ...
                        Text { color: Theme.lightGray1; font.bold: true; font.family: Theme.fontFamily; font.pixelSize: 20; text: root.phaseText(root.state.phase) }
                    }
                }
            }
            ...
            Row {
                ...
                Skin.Button {
                    ...
                    autoRepeat: true
                    onClicked: root.state.slower.trigger()
                }
                Skin.Button {
                    width: (parent.width - 12) / 3
                    height: 42
                    ...
                    enabled: root.canEdit
                    autoRepeat: true
                    onClicked: root.state.faster.trigger()
                }
                Skin.Button {
                    width: (parent.width - 12) / 3
                    height: 42
                    text: "TAP BPM"
                    enabled: root.canEdit
                    onClicked: root.state.tap.trigger()
                }
            }
            ...
            Row {
                ...
                Skin.Button {
                    ...
                    enabled: root.canEdit
                    autoRepeat: true
                    onClicked: root.state.earlier.trigger()
                }
                Skin.Button {
                    width: (parent.width - 18) / 4
                    height: 42
                    ...
                    enabled: root.canEdit
                    autoRepeat: true
                    onClicked: root.state.later.trigger()
                }
                Skin.Button {
                    width: (parent.width - 18) / 4
                    height: 42
                    text: "½ BEAT"
                    enabled: root.canEdit
                    onClicked: root.state.half.trigger()
                }
                Skin.Button {
                    width: (parent.width - 18) / 4
                    height: 42
                    ...
                    enabled: root.canEdit
                    highlight: true
                    onClicked: root.state.align.trigger()
                }
            }
            ...
            Row {
                ...
                Skin.Button {
                    ...
                    text: "MATCH DECK"
                    enabled: root.canEdit
                    onClicked: root.state.match.trigger()
                }
                Skin.Button {
                    width: (parent.width - 6) / 2
                    height: 42
                    text: "UNDO"
                    enabled: root.controlsInitialized && root.state.undoPossible.value > 0
                    onClicked: root.state.undo.trigger()
                }
            }
            ...
            Flow {
                ...
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "½×"; enabled: root.canEdit; onClicked: root.state.halve.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "⅔×"; enabled: root.canEdit; onClicked: root.state.twoThirds.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "¾×"; enabled: root.canEdit; onClicked: root.state.threeFourths.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "⅘×"; enabled: root.canEdit; onClicked: root.state.fourFifths.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "5/4×"; enabled: root.canEdit; onClicked: root.state.fiveFourths.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "4/3×"; enabled: root.canEdit; onClicked: root.state.fourThirds.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "3/2×"; enabled: root.canEdit; onClicked: root.state.threeHalves.trigger() }
                Skin.Button { width: (parent.width - 18) / 4; height: 42; text: "2×"; enabled: root.canEdit; onClicked: root.state.doubleBpm.trigger() }
            }
            ...
            Row {
                ...
                Skin.Button {
                    width: (parent.width - 6) / 2
                    height: 42
                    text: root.state.lock.value > 0.5 ? "UNLOCK GRID" : "LOCK GRID"
                    highlight: root.state.lock.value > 0.5
                    enabled: root.controlsInitialized
                    onClicked: root.state.lock.trigger()
                }
                Text {
                    width: (parent.width - 6) / 2
                    height: 42
                    ...
                    text: !root.state.loaded ? "No track loaded" :
                          root.state.lock.value > 0.5 ? "BeatGrid locked" :
